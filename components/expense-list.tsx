...
export function ExpenseList({ expenses, onDelete }: ExpenseListProps) {
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      Travel: "bg-blue-100 text-blue-800", "Outdoor Food": "bg-green-100 text-green-800", Fun: "bg-purple-100 text-purple-800", Gift: "bg-pink-100 text-pink-800", Essentials: "bg-gray-100 text-gray-800", Personal: "bg-indigo-100 text-indigo-800", Housing: "bg-orange-100 text-orange-800", Utilities: "bg-yellow-100 text-yellow-800", Groceries: "bg-emerald-100 text-emerald-800", Transportation: "bg-cyan-100 text-cyan-800", "Phone/Internet": "bg-teal-100 text-teal-800", Household: "bg-lime-100 text-lime-800", Insurance: "bg-red-100 text-red-800", "Children/Pets": "bg-rose-100 text-rose-800", Leisure: "bg-violet-100 text-violet-800", Holidays: "bg-fuchsia-100 text-fuchsia-800", "Debt Repayments": "bg-slate-100 text-slate-800", Miscellaneous: "bg-neutral-100 text-neutral-800",
    }
    ...
    <Card className="shadow-lg border-border/50 backdrop-blur-sm">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => setShowCalculations(!showCalculations)} className="flex items-center gap-2">
            ...
            <TableBody>
              {expenses.map((expense) => (
                <TableRow key={expense.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className="text-xs">{expense.month}</Badge>
                  </TableCell>
                  <TableCell>
                    <Badge className={`text-xs ${getCategoryColor(expense.category)}`}>{expense.category}</Badge>
                  </TableCell>
                  <TableCell className="max-w-[200px]">
                    <div className="truncate" title={expense.description}>{expense.description}</div>
                  </TableCell>
                  <TableCell className="text-right font-medium">{formatCurrency(expense.amount)}</TableCell>
                  <TableCell>
                    <Badge variant={expense.paidBy === "Both" ? "secondary" : "default"} className="text-xs">{expense.paidBy}</Badge>
                  </TableCell>
                  <TableCell>
